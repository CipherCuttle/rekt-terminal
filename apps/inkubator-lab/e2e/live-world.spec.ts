...
async function captureEvidence(page: Page, filePath: string, label: string) {
  if (!process.env.WORLD_EVIDENCE_DIR) return;
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.evaluate((stampText) => {
    const stamp = document.createElement('div');
    stamp.dataset.worldEvidenceStamp = 'true';
  ...
  await page.locator('[data-world-evidence-stamp="true"]').evaluate((stamp) => stamp.remove());
}
...
test('LIVE WORLD renders NOW plus a truthful signal tape and animates only a new feed signal', async ({page}) => {
  ...
  await expect(page.getByText('EXTERNAL TEST RECORDED', {exact: true})).toBeVisible();
...
test('LIVE WORLD keeps available public projects visible and preserves mobile causal order', async ({page}) => {
  await page.setViewportSize({width: 390, height: 844});
  await page.emulateMedia({reducedMotion: 'reduce'});
