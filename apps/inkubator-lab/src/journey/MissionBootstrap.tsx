...
export default function MissionBootstrap({client = createInkubatorApiClient()}: {client?: Pick<InkubatorApiClient, 'listRounds' | 'joinRound' | 'createMission'>} = {}) {
  ...
  return <TerminalShell mode="COMMAND" kicker="01 / DECLARE" title="What are you shipping?" description="Two short decisions create the Mission. Then connect the repo and keep building normally; Inkubator tracks the work around it." workspaceClassName="journey-workspace">
    <section className="journey-display"><small>COMMAND / NO ACTIVE MISSION</small><h2>{step === 1 ? 'Name the thing.' : 'Define real.'}</h2><p>{step === 1 ? 'Tell Inkubator what this build is. You will keep doing the actual work in your normal repo and editor.' : 'Say what must work for this to count as shipped, then choose the first concrete move you will carry back into the build.'}</p><span>CLAIMED ≠ OBSERVED ≠ PROVEN</span></section>
    <form className="journey-form" onSubmit={submit} onChange={resetMutation}>
      ...
      {rounds.isPending ? <p role="status">Loading available Rounds…</p> : rounds.isError ? <p role="alert">Rounds unavailable. <button type="button" onClick={() => void rounds.refetch()}>Retry</button></p> : !openRounds.length ? <p role="status">No Round is open. Your identity is saved; return when a Round opens.</p> : step === 1 ? <>
        ...
        <p className="journey-step-note">This becomes both your current focus and first Next Move. COMMAND will keep that move visible while you work in the repo; you can split focus and Next Move later if the build changes.</p>
