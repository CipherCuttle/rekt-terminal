...
export const EVIDENCE_POLICY_FOR_ENVIRONMENT: Record<MarketEnvironment, EvidencePolicy> = { LIVE: 'LIVE_ONLY', DEMO: 'DEMO_ALLOW_SYNTHETIC' };
...
export interface PracticeStoreOptions { sessionId?: string; environment?: MarketEnvironment; now?: () => number; storage?: PracticeStorage; getQuote?: () => PracticeQuote | null; persistDebounceMs?: number; }
...
export class PracticeSessionStore {
  ...
  setQuoteProvider(getQuote: () => PracticeQuote | null): void { this.getQuote = getQuote; }
  ...
  hydrate(): Promise<void> { return (this.hydration ??= this.runHydration()); }
  private async runHydration(): Promise<void> {
    let loaded: unknown = null;
    try { loaded = await this.storage.load(); } catch { loaded = null; }
    if (loaded === null || loaded === undefined) { this.commit({ hydrated: true, restoreStatus: 'FRESH' }, false); return; }
    try {
  ...
  private schedulePersist(): void {
    ...
    this.persistTimer = setTimeout(() => { this.persistTimer = null; void this.persistNow(); }, this.persistDebounceMs);
  }
  ...
  submit(intent: PracticeIntent): PracticeIntentResult {
    ...
    if (position && position.instrumentId !== gate.observation.instrumentId) return this.reject('INSTRUMENT_LOCKED', `An open position on ${position.instrumentId} must be closed before trading another instrument.`);
  ...
  private submitRiskPlannedEntry(intent: Extract<PracticeIntent, { kind: 'BUY_RISK_PLANNED' }>, observation: MarketObservation, eventTimeMs: number): PracticeIntentResult {
    const { sim } = this.snapshot;
    ...
    const entry = executeSpotAction(planned.state, { type: 'BUY', intentId: `${sim.sessionId}:i${ordinal}`, fillId: `${sim.sessionId}:f${ordinal}`, eventTimeMs, observation, quoteNotionalWei: plan.plannedNotionalWei, config: DEFAULT_SPOT_FILL_CONFIG });
    if (!entry.accepted) {
  ...
  private applyRiskPlannedCommit(nextSim: SimState, instrumentId: string, evidenceProvenance: ProvenanceState, plan: RiskPlan, rejection: PracticeRejection | null): void {
    let career = reduceCareer(this.snapshot.career, { type: 'RISK_PLAN_CREATED', eventId: `${plan.planId}:career`, sourceReceiptId: `${plan.planId}:risk-plan`, planId: plan.planId, evidenceProvenance });
    if (nextSim.activeStop) career = reduceCareer(career, { type: 'STOP_PLACED', eventId: `${nextSim.activeStop.stopId}:career`, sourceReceiptId: `${nextSim.activeStop.stopId}:placed`, evidenceProvenance });
  ...
  private applyAccepted(intent: PracticeIntent, nextSim: SimState, instrumentId: string, evidenceProvenance: ProvenanceState): void {
    ...
    for (const summary of newSummaries) {
      careerEvents.push({
        type: 'TRADE_CLOSED', eventId: `${nextSim.sessionId}:${summary.tradeId}:closed`, sourceReceiptId: `${nextSim.sessionId}:${summary.tradeId}`,
        summary: {
          tradeId: summary.tradeId, sessionId: summary.sessionId, mode: summary.mode, realizedPnlWei: summary.realizedPnlWei,
          accountEquityAtCloseWei: summary.accountEquityAtCloseWei, lossBpsOfThenCurrentEquity: summary.lossBpsOfThenCurrentEquity,
          accountEquityAtOpenWei: summary.accountEquityAtOpenWei, maxDrawdownBpsAtClose: summary.maxDrawdownBpsAtClose,
          exitReason: summary.exitReason, stopUsed: summary.stopUsed, partialExitUsed: summary.partialExitUsed, liquidated: summary.liquidated,
          openedAtMs: summary.openedAtMs, firstStopPlacedAtMs: summary.firstStopPlacedAtMs, stopWidened: summary.stopWidened,
          riskPlanned: summary.riskPlan !== null, riskBudgetViolated: summary.riskBudgetViolated, riskBudgetVerified: summary.riskBudgetVerified,
      ...
      if (summary.riskPlan !== null && (summary.riskBudgetViolated || summary.riskBudgetVerified)) careerEvents.push({ type: summary.riskBudgetViolated ? 'RISK_BUDGET_VIOLATED' : 'RISK_BUDGET_RESPECTED', eventId: `${nextSim.sessionId}:${summary.tradeId}:risk-budget`, sourceReceiptId: `${nextSim.sessionId}:${summary.tradeId}`, tradeId: summary.tradeId, evidenceProvenance: summary.evidenceProvenance });
    }
  ...
  markToMarket(): boolean {
    ...
    const eventTimeMs = this.eventTime();
    const gate = evaluatePracticeEligibility(quote, eventTimeMs, { evidencePolicy: this.snapshot.sim.evidencePolicy });
    if (gate.status === 'BLOCKED' || gate.observation.instrumentId !== sim.position.instrumentId) return false;
  ...
  dismissTradeReview(): void { if (this.snapshot.tradeReview) this.commit({ tradeReview: null }, false); }
  clearRejection(): void { if (this.snapshot.lastRejection) this.commit({ lastRejection: null }, false); }

  private careerAfterAccountReset(): CareerState {
    const { career } = this.snapshot;
    return reduceCareer(career, { type: 'ACCOUNT_RESET_USED', eventId: `${career.careerId}:account-reset:${career.processedEventIds.length + 1}` });
  }

  resetSession(): void {
    const startedAtMs = this.now();
    const career = this.careerAfterAccountReset();
    this.commit({ sim: openedSession(`practice-${startedAtMs}`, startedAtMs, this.snapshot.environment), career, instrumentId: null, lastRejection: null, tradeReview: null, restoreStatus: 'FRESH' });
  }
  ...
  setEnvironment(environment: MarketEnvironment): void {
    ...
    const startedAtMs = this.now();
    const career = this.careerAfterAccountReset();
    this.commit({ environment, sim: openedSession(`practice-${environment.toLowerCase()}-${startedAtMs}`, startedAtMs, environment), career, instrumentId: null, lastRejection: null, tradeReview: null, restoreStatus: 'FRESH' });
  }
