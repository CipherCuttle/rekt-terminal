...
function phase5Error(reply: FastifyReply, cause: unknown) {
  ...
  if (message === 'authorization_denied' || message.endsWith('_self_forbidden') || message === 'social_interaction_blocked' || message === 'moderation_operator_required') return error(reply, 403, message);
