...
48. `STAGE_J3_REPRODUCIBLE_DEPLOYMENT_IDENTITY_V1.md` — reproducible bytecode/release/deployment identity procedure.
...
57. `STAGE_J4_RUNTIME_IDENTITY_REPAIR_RECEIPT_V1.md` — pre-audit repair receipt for immutable-aware release/deployment runtime identity; supersedes the old unpatched-runtime comparison.
58. `STAGE_J4_SWEDEN_EU_PRODUCTION_LEGAL_MATRIX_V1.md` — Swedish/EU counsel/accounting decision matrix; legal gate remains open.
59. `STAGE_J4_SIGNER_CEREMONY_REHEARSAL_V1.md` — dummy/ephemeral signer ceremony and loss/compromise tabletop; production signers remain unauthorized.
60. `STAGE_J4_POST_AUDIT_DEPLOYMENT_ATTESTATION_REHEARSAL_V1.md` — immutable-aware M04 rehearsal law; local fork does not satisfy real deployment attestation.
61. `STAGE_J4_PRODUCTION_READINESS_REHEARSAL_RECEIPT_V1.md` — durable readiness-v2 local-fork/dummy-signer PASS receipt bound to release schema 1.1.
62. `REKT_TECHNICAL_FACEPLATE_V1.md` — highest visual execution authority; it may not override product truth/trust/API semantics.
...
Stage J4 production-candidate vault            ACTIVE / RUNTIME IDENTITY REPAIR PASS / AUDIT HANDOFF READY / AUDIT NOT STARTED
Stage J4 readiness rehearsal                    PASS / PR #134 DRAFT UNMERGED / M04 NOT RUN
Stage J4 legal classification                   OPEN / ISSUE #132
...
- **Stage J4 superseded pre-repair audit scope:** `05e345181dfde2c720874b1fc2d1ee7dd39272a9` — historical green evidence retained, but this commit is no longer the external-audit target because its release/M04 runtime comparison did not account for Solidity immutable patching.
- **Stage J4 runtime-identity repair / external-audit code scope:** `56eaa98497f4c036227c7e2512dadeb640eacfe4` — PR #133 draft/open/unmerged; J4 push `35462005787` PASS; J4 PR `35462015111` PASS; J2 PR `35462015130` PASS; repaired-line documentation tip `25abd0cb6880dd6d1589e18a11997fdf17876ad3` has J4/J2/Inkubator/generic CI PASS; release receipt schema `inkubator.production-candidate-release/1.1`; immutable-normalized runtime-template hash + frozen immutable-layout digest + exhaustive deployment readback law; durable repair receipt `STAGE_J4_RUNTIME_IDENTITY_REPAIR_RECEIPT_V1.md`; external audit itself not started.
- **Stage J4 production-readiness rehearsal v2:** `107a8fa50d579f039e0d967fdefee2a50beeb807` — PR #134 draft/open/unmerged; readiness workflow `35464257886` PASS; dummy outcome/2-of-3 resolver ceremony PASS; release-receipt v1.1 binding PASS; local immutable-normalized runtime identity + exact instance hashes + exhaustive constructor readback PASS; `M04 = NOT_RUN`; production signers not created; legal gate remains open; durable receipt `STAGE_J4_PRODUCTION_READINESS_REHEARSAL_RECEIPT_V1.md`.
...
J4 core implementation is verified at `f5875e3be51c21bf55fcf83583f9af5ca9b22403`; bounded hostile review `5253171614` found three High issues, all were repaired, and targeted rereview `5253190481` closed the original High findings. The pre-audit tranche is verified at `18998bfe485a420046596ebba8a132691f02c2ff`. A production-readiness rehearsal then exposed that exact deployed runtime cannot equal an unpatched compiler runtime template when Solidity immutables are present. The old scope `05e345181dfde2c720874b1fc2d1ee7dd39272a9` is therefore superseded for independent audit. The repaired external-auditor code scope is `56eaa98497f4c036227c7e2512dadeb640eacfe4` on PR #133: creation identity remains exact, runtime identity uses the frozen compiler immutable layout plus immutable-normalized template hash, future deployed instances additionally require exact two-provider runtime agreement and exhaustive immutable/business-state readback. J4 dedicated verification is green and durable repair evidence is in `STAGE_J4_RUNTIME_IDENTITY_REPAIR_RECEIPT_V1.md`. Cross-language known answers, controlled native-USDC fork behavior, live read-only proxy/admin identity, release differentiation/reproducibility, bounded ERC-1271 gas grief, payout conflict/order, signer-request idempotency, append-only corrections and privacy/secret boundaries remain technical PASS. A readiness-v2 rehearsal on PR #134 then exercised the repaired release receipt against an isolated local Ink fork with ephemeral dummy signers and passed; that evidence is durable in `STAGE_J4_PRODUCTION_READINESS_REHEARSAL_RECEIPT_V1.md`. It explicitly leaves `M04 = NOT_RUN`, production signers uncreated, and the Swedish/EU legal gate open. J4 is not stage-closed: external audit is not started; deployed-code attestation is post-audit/pre-funding; actual production signer ceremony and legal/accounting/privacy/eligibility gates remain before production authority.
