...
export function buildStageJ4ReleaseCandidateReceipt({
  ...
  const payload = {
    schema_version: 'inkubator.production-candidate-release/1.1',
    release_authority: 'CANDIDATE_ONLY_NO_MAINNET_NO_MONEY',
    runtime_identity_mode: 'IMMUTABLE_NORMALIZED_TEMPLATE_PLUS_EXHAUSTIVE_READBACK',
    source_commit,
    ...
    vault_runtime_template_hash: assertDigest(vault_runtime_template_hash, 'vault normalized runtime template hash'),
    vault_immutable_layout_digest: assertDigest(vault_immutable_layout_digest, 'vault immutable layout digest'),
    resolver_creation_bytecode_hash: assertDigest(resolver_creation_bytecode_hash, 'resolver creation bytecode hash'),
    resolver_runtime_template_hash: assertDigest(resolver_runtime_template_hash, 'resolver normalized runtime template hash'),
    resolver_immutable_layout_digest: assertDigest(resolver_immutable_layout_digest, 'resolver immutable layout digest'),
    resolver_signer_set_digest: assertDigest(resolver_signer_set_digest, 'resolver signer-set digest'),
    resolver_quorum: 2,
    chain_id: STAGE_J4_NETWORK.chain_id,
