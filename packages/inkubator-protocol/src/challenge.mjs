...
function assertAllowedKeys(value, allowed, label) {
  for (const key of Object.keys(value)) {
    invariant(allowed.has(key), `${label} contains undeclared property ${key}`);
  }
}
...
const BUILD_CONTRACT_KEYS = new Set([
  'schema_version', 'challenge_id', 'contract_version', 'mechanism_version',
  'settlement_policy_version', 'ip_terms_version', 'title', 'brief',
  'outcome_contract', 'production_envelope', 'delivery_contract', 'preferences',
  'reference_architecture', 'normative_constraints', 'normative_references',
  'informational_references', 'knowledge', 'slot_limit', 'activation_minimum',
  'entry_deadline', 'build_start', 'submission_deadline', 'appeal_deadline',
...
const SUBMISSION_KEYS = new Set([
  'schema_version', 'challenge_id', 'entry_id', 'terms_digest', 'submission_version',
  'immutable_source_reference', 'artifact_digest', 'evidence_references',
  'optional_live_url', 'accepted_at',
]);
...
function assertCriterion(criterion, label) {
  assertObject(criterion, label);
  assertAllowedKeys(criterion, CRITERION_KEYS, label);
  assertString(criterion.id, `${label}.id`);
  assertString(criterion.description, `${label}.description`);
...
function assertReference(reference, label) {
  ...
  assertAllowedKeys(reference, REFERENCE_KEYS, label);
  assertString(reference.id, `${label}.id`);
  assertString(reference.kind, `${label}.kind`);
  assertHexDigest(reference.content_digest, `${label}.content_digest`);
  if ('source_url' in reference) assertString(reference.source_url, `${label}.source_url`);
}

function assertKnowledgeItem(item, label) {
  assertObject(item, label);
  assertAllowedKeys(item, KNOWLEDGE_KEYS, label);
  invariant(['KNOWN', 'ASSUMED', 'UNKNOWN'].includes(item.kind), `${label}.kind invalid`);
  assertString(item.key, `${label}.key`);
  invariant(typeof item.material === 'boolean', `${label}.material must be boolean`);
}

export function assertBuildContract(contract) {
  assertObject(contract, 'build contract');
  assertAllowedKeys(contract, BUILD_CONTRACT_KEYS, 'build contract');

  for (const key of [
    'schema_version', 'challenge_id', 'contract_version', 'mechanism_version',
    'settlement_policy_version', 'ip_terms_version', 'title', 'brief', 'settlement_asset',
  ]) assertString(contract[key], `build contract.${key}`);
  ...
  assertObject(contract.preferences, 'build contract.preferences');
  assertObject(contract.reference_architecture, 'build contract.reference_architecture');
  ...
  if ('informational_references' in contract) {
    ...
    contract.informational_references.forEach((reference, index) => {
      assertObject(reference, `build contract.informational_references[${index}]`);
      assertString(reference.id, `build contract.informational_references[${index}].id`);
      assertString(reference.url, `build contract.informational_references[${index}].url`);
    });
  }
  ...
  for (const key of ['entry_deadline', 'build_start', 'submission_deadline', 'appeal_deadline', 'review_deadline']) {
    assertSafeInt(contract[key], `build contract.${key}`, {min: 0});
  }
...
export function validateBuildContractReady(contract, registry = DEFAULT_VERSION_REGISTRY) {
  ...
  invariant(contract.submission_deadline <= contract.appeal_deadline, 'appeal_deadline must be at/after submission_deadline');
  invariant(contract.appeal_deadline <= contract.review_deadline, 'review_deadline must be at/after appeal_deadline');

  const criterionIds = allNormativeCriteria(contract).map((criterion) => criterion.id);
  stableUnique(criterionIds, 'normative criterion ids');
...
export function canonicalBuildContractPayload(contract) {
  assertBuildContract(contract);
  const payload = deepClone(contract);
  delete payload.terms_digest;
  return payload;
}

export function digestBuildContract(contract) {
  return digestRecord(canonicalBuildContractPayload(contract));
}

export function freezeBuildContract(contract, registry = DEFAULT_VERSION_REGISTRY) {
  validateBuildContractReady(contract, registry);
  const frozen = deepClone(contract);
  frozen.terms_digest = digestBuildContract(frozen);
  return deepFreeze(frozen);
}

export function assertFrozenBuildContract(contract, registry = DEFAULT_VERSION_REGISTRY) {
  validateBuildContractReady(contract, registry);
  assertHexDigest(contract.terms_digest, 'build contract.terms_digest');
  invariant(contract.terms_digest === digestBuildContract(contract), 'terms_digest does not match frozen Build Contract');
  return contract;
}

export function assertContractMatchesTermsDigest(contract, expectedDigest) {
  assertFrozenBuildContract(contract);
  invariant(contract.terms_digest === expectedDigest, 'Build Contract terms digest mismatch');
  return contract;
}

export function validateFundingFact(contract, fact) {
  validateBuildContractReady(contract);
  assertObject(fact, 'funding fact');
  invariant(fact.status === 'CONFIRMED', 'funding fact is not confirmed');
  invariant(fact.challenge_id === contract.challenge_id, 'funding fact Challenge mismatch');
  invariant(fact.asset === contract.settlement_asset, 'funding fact asset mismatch');
  invariant(fact.amount_minor_units === contract.prize_minor_units, 'funding fact amount mismatch');
  invariant(fact.contract_digest === digestBuildContract(contract), 'funding fact contract digest mismatch');
  return fact;
}

export function migrateDraftBuildContract(challenge, patch, registry = DEFAULT_VERSION_REGISTRY) {
  assertObject(challenge, 'challenge');
  invariant(challenge.status === 'DRAFT', 'only DRAFT Challenge contracts may migrate');
  assertObject(patch, 'contract patch');
  const candidate = {...deepClone(challenge.contract), ...deepClone(patch)};
  delete candidate.terms_digest;
  validateBuildContractReady(candidate, registry);
  return deepFreeze({...deepClone(challenge), contract: candidate});
}

export function canTransitionChallenge(from, to) {
  invariant(CHALLENGE_STATES.includes(from), `unknown Challenge state ${from}`);
  invariant(CHALLENGE_STATES.includes(to), `unknown Challenge state ${to}`);
  return LEGAL_TRANSITIONS[from].includes(to);
}

export function transitionChallenge(challenge, to, context = {}) {
  assertObject(challenge, 'challenge');
  assertString(challenge.challenge_id, 'challenge.challenge_id');
  ...
  if (to === 'ENTRY_OPEN') {
    assertFrozenBuildContract(contract, context.versionRegistry ?? DEFAULT_VERSION_REGISTRY);
  ...
  if (to === 'SUBMISSIONS_LOCKED') {
    assertSafeInt(context.now, 'now', {min: 0});
    invariant(context.now >= contract.submission_deadline, 'submission deadline not reached');
  }
  if (to === 'APPEAL_WINDOW') invariant(context.allFirstPassComplete === true, 'first-pass qualification is incomplete');
  if (to === 'FINAL_QUALIFIERS') {
    assertSafeInt(context.now, 'now', {min: 0});
    invariant(context.now >= contract.appeal_deadline, 'appeal deadline not reached');
    invariant(context.appealsResolved === true, 'appeals not fully resolved');
  }
  if (to === 'SELECTION') {
    invariant(Array.isArray(context.finalQualifierIds) && context.finalQualifierIds.length > 0, 'selection requires at least one final qualifier');
  }
  if (challenge.status === 'FINAL_QUALIFIERS' && to === 'SETTLEMENT_PENDING') {
    ...
    invariant(context.settlementIntent?.type === 'REFUND_NO_QUALIFIER', 'zero-qualifier path requires refund intent');
  }
  if (to === 'DEFAULT_RESOLUTION') {
    assertSafeInt(context.now, 'now', {min: 0});
    invariant(context.now >= contract.review_deadline, 'review deadline not reached');
    invariant(context.hasValidSelection !== true, 'valid organizer selection already exists');
  }
  if (to === 'SETTLEMENT_PENDING') {
    invariant(isObject(context.settlementIntent), 'settlement intent required');
  ...
  if (to === 'SETTLED') applyFinalizedSettlementFact(context.settlementIntent, context.executionFact, context.existingExecutionFact);
  if (to === 'RECEIPT_FILED') invariant(isObject(context.receipt), 'receipt required');

  return deepFreeze({...deepClone(challenge), status: to});
}

export function assertEntry(entry) {
  assertObject(entry, 'entry');
  assertAllowedKeys(entry, ENTRY_KEYS, 'entry');
  assertString(entry.entry_id, 'entry.entry_id');
  assertString(entry.builder_id, 'entry.builder_id');
  assertString(entry.payout_id, 'entry.payout_id');
  invariant(ENTRY_STATES.includes(entry.state), `unknown entry state ${entry.state}`);
  if ('build_start' in entry) assertSafeInt(entry.build_start, 'entry.build_start', {min: 0});
  if ('submission_deadline' in entry) assertSafeInt(entry.submission_deadline, 'entry.submission_deadline', {min: 0});
  return entry;
}
...
export function validateEntrySet(entries, {organizerBuilderId, organizerPayoutId, funderPayoutId} = {}) {
  ...
  stableUnique(entries.map((entry) => entry.entry_id), 'entry ids');
...
export function activateEntries(entries, contract, policy = {}) {
  assertFrozenBuildContract(contract);
  validateEntrySet(entries, policy);
  const activeSeatCount = entries.filter((entry) => entry.state === 'SEATED').length;
  invariant(activeSeatCount >= contract.activation_minimum, 'activation minimum not met');
  invariant(activeSeatCount <= contract.slot_limit, 'slot limit exceeded');
  return entries
    .map((entry) => entry.state === 'SEATED'
      ? {...deepClone(entry), state: 'ACTIVE', build_start: contract.build_start, submission_deadline: contract.submission_deadline}
      : deepClone(entry))
    .sort((a, b) => byteCompare(a.entry_id, b.entry_id));
}

export function publicEntryProjection(entry, {revealed = false} = {}) {
  assertEntry(entry);
  if (revealed) return deepClone(entry);
  return {entry_id: entry.entry_id, state: entry.state};
}

export function assertSubmissionManifest(manifest) {
  assertObject(manifest, 'submission manifest');
  assertAllowedKeys(manifest, SUBMISSION_KEYS, 'submission manifest');
  ...
  invariant(manifest.schema_version === 'inkubator.submission-manifest/1.0', `unsupported submission schema ${manifest.schema_version}`);
  assertHexDigest(manifest.terms_digest, 'submission manifest.terms_digest');
  assertHexDigest(manifest.artifact_digest, 'submission manifest.artifact_digest');
  assertSafeInt(manifest.submission_version, 'submission manifest.submission_version', {min: 1});
  assertSafeInt(manifest.accepted_at, 'submission manifest.accepted_at', {min: 0});
  assertObject(manifest.immutable_source_reference, 'submission manifest.immutable_source_reference');
  assertAllowedKeys(manifest.immutable_source_reference, SOURCE_REFERENCE_KEYS, 'submission manifest.immutable_source_reference');
  invariant(['GIT_COMMIT', 'CONTENT_ADDRESS', 'ARCHIVE_DIGEST'].includes(manifest.immutable_source_reference.kind), 'immutable source reference kind invalid');
  assertString(manifest.immutable_source_reference.value, 'submission manifest.immutable_source_reference.value');
  invariant(Array.isArray(manifest.evidence_references), 'submission manifest.evidence_references must be an array');
  manifest.evidence_references.forEach((value, index) => assertString(value, `submission manifest.evidence_references[${index}]`));
  if ('optional_live_url' in manifest) assertString(manifest.optional_live_url, 'submission manifest.optional_live_url');
  return manifest;
}
...
export function isSubmissionEligible(manifest, contract, entryId) {
  try {
    ...
    assertFrozenBuildContract(contract);
    return manifest.challenge_id === contract.challenge_id
      && manifest.entry_id === entryId
      && manifest.terms_digest === contract.terms_digest
      && manifest.accepted_at <= contract.submission_deadline;
  } catch {
    return false;
  }
}
...
export function selectFinalSubmission(manifests, contract, entryId) {
  invariant(Array.isArray(manifests), 'manifests must be an array');
  const eligible = manifests.filter((manifest) => isSubmissionEligible(manifest, contract, entryId));
  const versions = eligible.map((manifest) => manifest.submission_version);
  stableUnique(versions.map(String), 'eligible submission versions');
  if (eligible.length === 0) return null;
  eligible.sort((a, b) => (a.accepted_at - b.accepted_at) || (a.submission_version - b.submission_version));
  return deepClone(eligible.at(-1));
}

export function recordArchiveObservation(manifest, archiveState, reason = null) {
  assertSubmissionManifest(manifest);
  invariant(ARCHIVE_STATES.includes(archiveState), `unknown archive state ${archiveState}`);
  return deepFreeze({
...
export function computeQualification(contract, criterionResults) {
  assertFrozenBuildContract(contract);
  invariant(Array.isArray(criterionResults), 'criterionResults must be an array');
  const mandatory = allNormativeCriteria(contract).filter((criterion) => criterion.mandatory);
  const mandatoryIds = stableUnique(mandatory.map((criterion) => criterion.id), 'mandatory criterion ids');
  ...
  for (const result of criterionResults) {
    assertObject(result, 'criterion result');
    assertString(result.criterion_id, 'criterion result.criterion_id');
    invariant(QUALIFICATION_RESULTS.includes(result.result), `invalid criterion result ${result.result}`);
  }
  const values = criterionResults.map((result) => result.result);
  const overall = values.includes('DISPUTED') ? 'DISPUTED' : values.includes('FAIL') ? 'NOT_QUALIFIED' : 'QUALIFIED';
  return deepFreeze({overall, criteria: deepClone(criterionResults)});
}

export function appendAppealEvent(history, event) {
  invariant(Array.isArray(history), 'appeal history must be an array');
  assertObject(event, 'appeal event');
  invariant(['APPEAL', 'RESOLUTION'].includes(event.type), 'appeal event type invalid');
  invariant(!('new_requirement' in event), 'appeal cannot introduce a new requirement');
  const appealCount = history.filter((item) => item.type === 'APPEAL').length;
  const resolutionCount = history.filter((item) => item.type === 'RESOLUTION').length;
  if (event.type === 'APPEAL') {
    invariant(appealCount === 0 && resolutionCount === 0, 'appeal budget exhausted');
  } else {
    invariant(appealCount === 1 && resolutionCount === 0, 'resolution requires exactly one unresolved appeal');
  }
  return deepFreeze([...deepClone(history), deepClone(event)]);
}

export function validateSelection(selectedEntryId, finalQualifierIds) {
  assertString(selectedEntryId, 'selected entry id');
  invariant(Array.isArray(finalQualifierIds) && finalQualifierIds.length > 0, 'selection requires final qualifiers');
  const qualifiers = stableUnique(finalQualifierIds, 'final qualifier ids');
  invariant(qualifiers.includes(selectedEntryId), 'selected entry is not a final qualifier');
  return selectedEntryId;
}

export function computeDefaultDistribution(prizeMinorUnits, qualifierIds) {
  assertSafeInt(prizeMinorUnits, 'prize minor units', {min: 0});
  invariant(Array.isArray(qualifierIds) && qualifierIds.length >= 2, 'default distribution requires at least two qualifiers');
  const ordered = stableUnique(qualifierIds, 'final qualifier ids');
  const base = Math.floor(prizeMinorUnits / ordered.length);
  const remainder = prizeMinorUnits - (base * ordered.length);
...
export function computeDefaultResolution(finalQualifierIds, prizeMinorUnits) {
  invariant(Array.isArray(finalQualifierIds), 'finalQualifierIds must be an array');
  const qualifiers = stableUnique(finalQualifierIds, 'final qualifier ids');
  assertSafeInt(prizeMinorUnits, 'prize minor units', {min: 0});
...
function canonicalRecipients(recipients) {
  ...
  stableUnique(result.map((recipient) => recipient.recipient_id), 'settlement recipient ids');
  return result;
}
...
export function buildSettlementIntent({contract, resolution, recipientByEntryId = {}, refundRecipientId = null}) {
  ...
  if (['REFUND_PRE_BUILD', 'REFUND_NO_QUALIFIER', 'CANCELLED_BY_RESOLUTION'].includes(resolution.type)) {
    assertString(refundRecipientId, 'refundRecipientId');
    recipients = [{recipient_id: refundRecipientId, amount_minor_units: contract.prize_minor_units}];
  } else {
    recipients = resolution.distributions.map(({entry_id, amount_minor_units}) => {
      const recipientId = recipientByEntryId[entry_id];
      assertString(recipientId, `recipient for entry ${entry_id}`);
      return {recipient_id: recipientId, amount_minor_units};
    });
  }
  recipients = canonicalRecipients(recipients);
  const total = recipients.reduce((sum, recipient) => sum + recipient.amount_minor_units, 0);
  assertSafeInt(total, 'settlement total', {min: 0});
  invariant(total === contract.prize_minor_units, 'settlement intent does not conserve full prize');
  return deepFreeze({
    ...
    recipients,
    winner_entry_id: resolution.winner_entry_id ?? null,
  });
}
...
function comparableSettlementShape(value) {
  return {
    ...
    settlement_policy_version: value.settlement_policy_version,
    asset: value.asset,
    total_minor_units: value.total_minor_units,
    recipients: canonicalRecipients(value.recipients),
  };
}

export function applyFinalizedSettlementFact(intent, fact, existingFact = null) {
  assertObject(intent, 'settlement intent');
  assertObject(fact, 'settlement execution fact');
  invariant(fact.finality === 'FINALIZED', 'settlement execution fact is not finalized');
  assertString(fact.execution_id, 'settlement execution fact.execution_id');
  ...
  if (existingFact) {
    invariant(digestRecord(existingFact) === digestRecord(normalized), 'conflicting finalized settlement fact');
    return existingFact;
  }
  return normalized;
}

export function computeIpTransferFact(intent, executionFact) {
  applyFinalizedSettlementFact(intent, executionFact);
  return intent.type === 'WINNER_PAYOUT' ? 'TRANSFER_TRIGGERED' : 'NOT_TRIGGERED';
}

export function terminalOutcomeFromIntent(intent) {
  if (intent.type === 'REFUND_PRE_BUILD') return 'REFUNDED_PRE_BUILD';
  if (intent.type === 'REFUND_NO_QUALIFIER') return 'REFUNDED_NO_QUALIFIER';
  if (intent.type === 'DEFAULT_DISTRIBUTION') return 'DEFAULT_DISTRIBUTED';
  if (intent.type === 'CANCELLED_BY_RESOLUTION') return 'CANCELLED_BY_RESOLUTION';
  if (intent.type === 'WINNER_PAYOUT') return 'WINNER_SETTLED';
  throw new Error(`unknown settlement intent type ${intent.type}`);
}

export function fileReceipt({contract, settlementIntent, settlementExecutionFact}) {
  assertFrozenBuildContract(contract);
  const executionFact = applyFinalizedSettlementFact(settlementIntent, settlementExecutionFact);
  const payload = {
    ...
    terminal_outcome: terminalOutcomeFromIntent(settlementIntent),
    settlement_intent: deepClone(settlementIntent),
    settlement_execution_fact: deepClone(executionFact),
    ip_transfer_fact: computeIpTransferFact(settlementIntent, executionFact),
  };
  const digest = digestRecord(payload);
  return deepFreeze({...payload, receipt_id: `receipt_${digest.slice(0, 32)}`, digest});
}

export function appendReceiptCorrection(receipts, {supersedes, reason, authority, evidence_refs = [], corrected_projection = {}}) {
  invariant(Array.isArray(receipts) && receipts.length > 0, 'receipts required');
  stableUnique(receipts.map((receipt) => receipt.receipt_id), 'receipt ids');
  const byId = new Map(receipts.map((receipt) => [receipt.receipt_id, receipt]));
  const previous = byId.get(supersedes);
  invariant(previous, 'superseded receipt not found');
  const seen = new Set();
  let cursor = previous;
  while (cursor) {
    invariant(!seen.has(cursor.receipt_id), 'receipt correction chain contains a cycle');
    seen.add(cursor.receipt_id);
    cursor = cursor.supersedes ? byId.get(cursor.supersedes) : null;
  }
  assertString(reason, 'correction reason');
  assertString(authority, 'correction authority');
  invariant(Array.isArray(evidence_refs), 'correction evidence_refs must be an array');
  evidence_refs.forEach((ref, index) => assertString(ref, `correction evidence_refs[${index}]`));
  assertObject(corrected_projection, 'corrected projection');
  for (const forbidden of ['settlement_intent', 'settlement_execution_fact', 'terms_digest', 'mechanism_version', 'settlement_policy_version', 'ip_terms_version']) {
    invariant(!(forbidden in corrected_projection), `correction cannot rewrite ${forbidden}`);
  }
  const payload = {
    ...
    corrected_projection: deepClone(corrected_projection),
  };
  const digest = digestRecord(payload);
  const correction = deepFreeze({...payload, receipt_id: `correction_${digest.slice(0, 32)}`, digest});
  invariant(correction.receipt_id !== supersedes, 'correction cannot supersede itself');
  return correction;
}

export function publicReceiptProjection(receipt) {
  assertObject(receipt, 'receipt');
  const projection = deepClone(receipt);
  delete projection.private_source;
  delete projection.private_source_metadata;
  return projection;
}
