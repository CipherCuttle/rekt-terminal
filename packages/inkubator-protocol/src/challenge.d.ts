...
export interface BuildContract {
  ...
  title: string;
  brief: string;
  outcome_contract: CriteriaContainer;
  production_envelope: CriteriaContainer;
  delivery_contract: CriteriaContainer;
  preferences: Record<string, unknown>;
  reference_architecture: Record<string, unknown>;
  normative_constraints: Criterion[];
...
export interface SubmissionManifest {
  ...
  immutable_source_reference: {kind: 'GIT_COMMIT' | 'CONTENT_ADDRESS' | 'ARCHIVE_DIGEST'; value: string};
...
export interface SettlementIntent {
  ...
  settlement_policy_version: string;
  asset: string;
  type: 'REFUND_PRE_BUILD' | 'REFUND_NO_QUALIFIER' | 'WINNER_PAYOUT' | 'DEFAULT_DISTRIBUTION' | 'CANCELLED_BY_RESOLUTION';
  total_minor_units: number;
  recipients: SettlementRecipient[];
  winner_entry_id: string | null;
}
export interface SettlementExecutionFact extends Omit<SettlementIntent, 'type' | 'winner_entry_id'> {
  finality: 'FINALIZED' | 'PENDING' | 'REORGED' | 'DISPUTED';
  execution_id: string;
}
...
export function assertBuildContract(contract: BuildContract): BuildContract;
...
export function assertFrozenBuildContract(contract: BuildContract, registry?: typeof DEFAULT_VERSION_REGISTRY): BuildContract;
...
export function transitionChallenge(challenge: {challenge_id: string; status: ChallengeState; contract: BuildContract}, to: ChallengeState, context?: Record<string, unknown>): unknown;
...
export function assertSubmissionManifest(manifest: SubmissionManifest): SubmissionManifest;
...
export function computeQualification(contract: BuildContract, criterionResults: Array<{criterion_id: string; result: QualificationCriterionResult}>): {overall: QualificationOverall; criteria: Array<{criterion_id: string; result: QualificationCriterionResult}>};
...
export function computeDefaultResolution(finalQualifierIds: string[], prizeMinorUnits: number): {type: string; winner_entry_id: string | null; distributions: Array<{entry_id: string; amount_minor_units: number}>};
export function buildSettlementIntent(args: {contract: BuildContract; resolution: {type: SettlementIntent['type']; winner_entry_id: string | null; distributions: Array<{entry_id: string; amount_minor_units: number}>}; recipientByEntryId?: Record<string, string>; refundRecipientId?: string | null}): SettlementIntent;
